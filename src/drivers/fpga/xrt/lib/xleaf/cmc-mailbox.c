...
static int cmc_mailbox_wait(struct xrt_cmc_mbx *cmc_mbx)
{
	...
	if (!cmc_pkt_host_owned(cmc_mbx)) {
		xrt_err(cmc_mbx->xdev, "CMC packet error: time'd out");
		return -ETIMEDOUT;
	}
	...
	if (val) {
		xrt_err(cmc_mbx->xdev, "CMC packet error: %d", val);
...
#ifdef	MBX_PKT_DEBUG
	xrt_info(cmc_mbx->xdev, "Sending CMC packet: %d DWORDS...", len);
	xrt_info(cmc_mbx->xdev, "opcode=%d payload_sz=0x%x (0x%x)",
...
static int cmc_mailbox_pkt_read(struct xrt_cmc_mbx *cmc_mbx)
{
	...
	if (hdr.payload_sz == 0 || len > CMC_PKT_MAX_SZ) {
		xrt_err(cmc_mbx->xdev, "read invalid CMC packet");
		return -EINVAL;
	}
...
{
	int ret;
	struct xrt_cmc_mbx *cmc_mbx = cmc_xdev2mbx(xdev);

	if (!cmc_mbx)
		return -EINVAL;

	if (cmc_mbx->generation != generation) {
		xrt_err(cmc_mbx->xdev, "stale generation number passed in");
		return -EINVAL;
	}
...
int cmc_mailbox_acquire(struct xrt_device *xdev)
{
	...
	if (down_killable(&cmc_mbx->sem)) {
		xrt_info(cmc_mbx->xdev, "giving up on acquiring CMC mailbox");
		return -ERESTARTSYS;
	}
...
int cmc_mailbox_probe(struct xrt_device *xdev, struct cmc_reg_map *regmaps, void **hdl)
{
	...
	cmc_mbx = devm_kzalloc(DEV(xdev), sizeof(*cmc_mbx), GFP_KERNEL);
	if (!cmc_mbx)
		return -ENOMEM;
	...
	if (cmc_mbx->mbx_offset == 0) {
		xrt_err(cmc_mbx->xdev, "CMC mailbox is not available");
		goto done;
	} else {
		xrt_info(cmc_mbx->xdev, "CMC mailbox offset is 0x%x", cmc_mbx->mbx_offset);
	}
