...
struct xmgnt_mailbox {
	struct xrt_device *xdev;
	struct xrt_device *mailbox;
	struct mutex lock; /* lock for xmgnt_mailbox */
	char *test_msg;
...
static inline void xmgnt_mailbox_prt_req(struct xmgnt_mailbox *xmbx, bool send,
...
#define XMGNT_MAILBOX_PRT_REQ_SEND(xmbx, req, sw_ch)			\
	xmgnt_mailbox_prt_req(xmbx, true, req, sw_ch)
#define XMGNT_MAILBOX_PRT_REQ_RECV(xmbx, req, sw_ch)			\
	xmgnt_mailbox_prt_req(xmbx, false, req, sw_ch)

static inline void xmgnt_mailbox_prt_resp(struct xmgnt_mailbox *xmbx,
...
static inline struct xmgnt_mailbox *xdev2mbx(struct xrt_device *xdev)
{
	return (struct xmgnt_mailbox *)xmgnt_xdev2mailbox(xdev);
}

static void xmgnt_mailbox_post(struct xmgnt_mailbox *xmbx,
			       u64 msgid, bool sw_ch, void *buf, size_t len)
{
	...
	if (msgid == 0)
		XMGNT_MAILBOX_PRT_REQ_SEND(xmbx, (struct xcl_mailbox_req *)buf, sw_ch);
	else
		xmgnt_mailbox_prt_resp(xmbx, &post);
...
static void xmgnt_mailbox_notify(struct xmgnt_mailbox *xmbx, bool sw_ch,
				 struct xcl_mailbox_req *req, size_t len)
{
	xmgnt_mailbox_post(xmbx, 0, sw_ch, req, len);
}

static void xmgnt_mailbox_respond(struct xmgnt_mailbox *xmbx,
				  u64 msgid, bool sw_ch, void *buf, size_t len)
{
	mutex_lock(&xmbx->lock);
	xmgnt_mailbox_post(xmbx, msgid, sw_ch, buf, len);
	mutex_unlock(&xmbx->lock);
}

static void xmgnt_mailbox_resp_test_msg(struct xmgnt_mailbox *xmbx, u64 msgid, bool sw_ch)
{
	...
	xmgnt_mailbox_respond(xmbx, msgid, sw_ch, msg, strlen(msg) + 1);
	vfree(msg);
}

static int xmgnt_mailbox_dtb_add_prop(struct xrt_device *xdev,
...
static int xmgnt_mailbox_dtb_add_vbnv(struct xrt_device *xdev, char *dtb)
{
	int rc = 0;
	char *vbnv = xmgnt_get_vbnv(xdev);

	if (!vbnv) {
		xrt_err(xdev, "failed to get VBNV");
		return -ENOENT;
	}
	rc = xmgnt_mailbox_dtb_add_prop(xdev, dtb, NULL, NULL, XRT_MD_PROP_VBNV,
					vbnv, strlen(vbnv) + 1);
	kfree(vbnv);
	return rc;
}

static int xmgnt_mailbox_dtb_copy_logic_uuid(struct xrt_device *xdev,
					     const char *src_dtb, char *dst_dtb)
{
	...
	return xmgnt_mailbox_dtb_add_prop(xdev, dst_dtb, NULL, NULL,
					  XRT_MD_PROP_LOGIC_UUID, val, sz);
}

static int xmgnt_mailbox_dtb_add_vrom(struct xrt_device *xdev,
				      const char *src_dtb, char *dst_dtb)
{
	...
	enum feature_bit_mask {
		...
		, BOARD_MGNT_ENBLD		= 0x0000000000000004
	...
	*(u32 *)header.entry_point_string = 0x786e6c78;
	...
	if (rc == 0)
		header.feature_bitmap |= BOARD_MGNT_ENBLD;
...
static u32 xmgnt_mailbox_dtb_user_pf(struct xrt_device *xdev,
...
static int xmgnt_mailbox_dtb_copy_user_endpoints(struct xrt_device *xdev,
						 const char *src, char *dst)
{
	int rc = 0;
	char *epname = NULL, *regmap = NULL;
	u32 pfnum = xmgnt_mailbox_dtb_user_pf(xdev, src, XRT_MD_NODE_MAILBOX_USER, NULL);
	...
	for (xrt_md_get_next_endpoint(dev, src, NULL, NULL, &epname, &regmap);
		...
		if (pfnum !=
			xmgnt_mailbox_dtb_user_pf(xdev, src, epname, regmap))
			continue;
...
static char *xmgnt_mailbox_user_dtb(struct xrt_device *xdev)
{
	const char *src = NULL;
	char *dst = NULL;
	struct device *dev = DEV(xdev);
	...
	rc = xmgnt_mailbox_dtb_add_vbnv(xdev, dst);
	if (rc)
		goto fail;

	src = xmgnt_get_dtb(xdev, XMGNT_BLP);
	if (!src) {
		xrt_err(xdev, "failed to get BLP dtb");
		goto fail;
	}

	rc = xmgnt_mailbox_dtb_copy_logic_uuid(xdev, src, dst);
	if (rc)
		goto fail;

	rc = xmgnt_mailbox_dtb_add_vrom(xdev, src, dst);
	if (rc)
		goto fail;
	...
	rc = xmgnt_mailbox_dtb_copy_user_endpoints(xdev, src, dst);
	if (rc)
		goto fail;
...
static void xmgnt_mailbox_resp_subdev(struct xmgnt_mailbox *xmbx,
				      u64 msgid, bool sw_ch, u64 offset, u64 size)
{
	struct xrt_device *xdev = xmbx->xdev;
	char *dtb = xmgnt_mailbox_user_dtb(xdev);
	long dtbsz;
	...
	xmgnt_mailbox_respond(xmbx, msgid, sw_ch, hdr, totalsz);

	vfree(dtb);
	vfree(hdr);
}

static void xmgnt_mailbox_resp_sensor(struct xmgnt_mailbox *xmbx,
				      u64 msgid, bool sw_ch, u64 offset, u64 size)
{
	struct xrt_device *xdev = xmbx->xdev;
	...
	xmgnt_mailbox_respond(xmbx, msgid, sw_ch, &sensors, min((u64)sizeof(sensors), size));
}

static int xmgnt_mailbox_get_freq(struct xmgnt_mailbox *xmbx,
...
static int xmgnt_mailbox_get_icap_idcode(struct xmgnt_mailbox *xmbx, u64 *id)
{
	struct xrt_device *xdev = xmbx->xdev;
...
static int xmgnt_mailbox_get_mig_calib(struct xmgnt_mailbox *xmbx, u64 *calib)
{
	struct xrt_device *xdev = xmbx->xdev;
...
static void xmgnt_mailbox_resp_icap(struct xmgnt_mailbox *xmbx,
				    u64 msgid, bool sw_ch, u64 offset, u64 size)
{
	struct xrt_device *xdev = xmbx->xdev;
	struct xcl_pr_region icap = { 0 };

	xmgnt_mailbox_get_freq(xmbx, CT_DATA, &icap.freq_data, &icap.freq_cntr_data);
	xmgnt_mailbox_get_freq(xmbx, CT_KERNEL, &icap.freq_kernel, &icap.freq_cntr_kernel);
	xmgnt_mailbox_get_freq(xmbx, CT_SYSTEM, &icap.freq_system, &icap.freq_cntr_system);
	xmgnt_mailbox_get_icap_idcode(xmbx, &icap.idcode);
	xmgnt_mailbox_get_mig_calib(xmbx, &icap.mig_calib);
	WARN_ON(sizeof(icap.uuid) != sizeof(uuid_t));
	xmgnt_get_provider_uuid(xdev, XMGNT_ULP, (uuid_t *)&icap.uuid);

	xmgnt_mailbox_respond(xmbx, msgid, sw_ch, &icap, min_t(u64, sizeof(icap), size));
}

static void xmgnt_mailbox_resp_bdinfo(struct xmgnt_mailbox *xmbx,
				      u64 msgid, bool sw_ch, u64 offset, u64 size)
{
	struct xrt_device *xdev = xmbx->xdev;
	...
	xmgnt_mailbox_respond(xmbx, msgid, sw_ch, info, min((u64)sizeof(*info), size));

	vfree(info);
}

static void xmgnt_mailbox_simple_respond(struct xmgnt_mailbox *xmbx, u64 msgid, bool sw_ch, int rc)
{
	xmgnt_mailbox_respond(xmbx, msgid, sw_ch, &rc, sizeof(rc));
}

static void xmgnt_mailbox_resp_peer_data(struct xmgnt_mailbox *xmbx, struct xcl_mailbox_req *req,
					 size_t len, u64 msgid, bool sw_ch)
{
	...
	switch (pdata->kind) {
	case XCL_SENSOR:
		xmgnt_mailbox_resp_sensor(xmbx, msgid, sw_ch, pdata->offset, pdata->size);
		break;
	case XCL_ICAP:
		xmgnt_mailbox_resp_icap(xmbx, msgid, sw_ch, pdata->offset, pdata->size);
		break;
	case XCL_BDINFO:
		xmgnt_mailbox_resp_bdinfo(xmbx, msgid, sw_ch, pdata->offset, pdata->size);
		break;
	case XCL_SUBDEV:
		xmgnt_mailbox_resp_subdev(xmbx, msgid, sw_ch, pdata->offset, pdata->size);
		break;
	case XCL_MIG_ECC:
	case XCL_FIREWALL:
	case XCL_DNA:
		xmgnt_mailbox_simple_respond(xmbx, msgid, sw_ch, 0);
		break;
	default:
...
static bool xmgnt_mailbox_is_same_domain(struct xmgnt_mailbox *xmbx,
					 struct xcl_mailbox_conn *mb_conn)
{
	...
	if (paddr != (phys_addr_t)mb_conn->paddr) {
		xrt_info(xdev, "paddrs differ, user 0x%llx, mgnt 0x%llx", mb_conn->paddr, paddr);
		return false;
	}

	crc_chk = crc32c_le(~0, (void *)mb_conn->kaddr, PAGE_SIZE);
	if (crc_chk != mb_conn->crc32) {
		xrt_info(xdev, "CRCs differ, user 0x%x, mgnt 0x%x", mb_conn->crc32, crc_chk);
		return false;
	}
...
static void xmgnt_mailbox_resp_user_probe(struct xmgnt_mailbox *xmbx, struct xcl_mailbox_req *req,
					  size_t len, u64 msgid, bool sw_ch)
{
	...
	if (xmgnt_mailbox_is_same_domain(xmbx, conn)) {
		xmbx->peer_in_same_domain = true;
		resp->conn_flags |= XCL_MB_PEER_SAME_DOMAIN;
	}

	xmgnt_mailbox_respond(xmbx, msgid, sw_ch, resp, sizeof(*resp));
	vfree(resp);
}

static void xmgnt_mailbox_resp_hot_reset(struct xmgnt_mailbox *xmbx, struct xcl_mailbox_req *req,
					 size_t len, u64 msgid, bool sw_ch)
{
	int ret;
	struct xrt_device *xdev = xmbx->xdev;
	...
	ret = xmgnt_hot_reset(xdev);
	if (ret)
		xrt_err(xdev, "failed to hot reset: %d", ret);
	else
		xmgnt_peer_notify_state(xmbx, true);
}

static void xmgnt_mailbox_resp_load_xclbin(struct xmgnt_mailbox *xmbx, struct xcl_mailbox_req *req,
					   size_t len, u64 msgid, bool sw_ch)
{
	...
	xmgnt_mailbox_simple_respond(xmbx, msgid, sw_ch, ret);
}

static void xmgnt_mailbox_listener(void *arg, void *data, size_t len,
				   u64 msgid, int err, bool sw_ch)
{
	struct xmgnt_mailbox *xmbx = (struct xmgnt_mailbox *)arg;
	struct xrt_device *xdev = xmbx->xdev;
	...
	XMGNT_MAILBOX_PRT_REQ_RECV(xmbx, req, sw_ch);
	switch (req->req) {
	case XCL_MAILBOX_REQ_TEST_READ:
		xmgnt_mailbox_resp_test_msg(xmbx, msgid, sw_ch);
		break;
	case XCL_MAILBOX_REQ_PEER_DATA:
		xmgnt_mailbox_resp_peer_data(xmbx, req, len, msgid, sw_ch);
		break;
	case XCL_MAILBOX_REQ_READ_P2P_BAR_ADDR:
		xmgnt_mailbox_simple_respond(xmbx, msgid, sw_ch, -ENOTSUPP);
		break;
	case XCL_MAILBOX_REQ_USER_PROBE:
		xmgnt_mailbox_resp_user_probe(xmbx, req, len, msgid, sw_ch);
		break;
	case XCL_MAILBOX_REQ_HOT_RESET:
		xmgnt_mailbox_resp_hot_reset(xmbx, req, len, msgid, sw_ch);
		break;
	case XCL_MAILBOX_REQ_LOAD_XCLBIN_KADDR:
		if (xmbx->peer_in_same_domain) {
			xmgnt_mailbox_resp_load_xclbin(xmbx, req, len, msgid, sw_ch);
		} else {
...
static void xmgnt_mailbox_reg_listener(struct xmgnt_mailbox *xmbx)
{
	struct xrt_mailbox_listen listen = { xmgnt_mailbox_listener, xmbx };
...
static void xmgnt_mailbox_unreg_listener(struct xmgnt_mailbox *xmbx)
{
...
{
	struct xmgnt_mailbox *xmbx = xdev2mbx(xdev);
	...
	case XRT_EVENT_POST_CREATION:
		...
		xmgnt_mailbox_reg_listener(xmbx);
		mutex_unlock(&xmbx->lock);
		break;
	case XRT_EVENT_PRE_REMOVAL:
		WARN_ON(!xmbx->mailbox);
		mutex_lock(&xmbx->lock);
		xmgnt_mailbox_unreg_listener(xmbx);
...
static ssize_t xmgnt_mailbox_user_dtb_show(struct file *filp, struct kobject *kobj,
					   ...
					   loff_t off, size_t count)
{
	...
	blob = xmgnt_mailbox_user_dtb(xdev);
	if (!blob) {
		ret = -ENOENT;
		goto failed;
	}
...
static struct bin_attribute meta_data_attr = {
	...
	.read = xmgnt_mailbox_user_dtb_show,
	.size = 0
};

static struct bin_attribute  *xmgnt_mailbox_bin_attrs[] = {
	&meta_data_attr,
	NULL,
};

static int xmgnt_mailbox_get_test_msg(struct xmgnt_mailbox *xmbx, bool sw_ch,
				      char *buf, size_t *len)
{
	int rc;
	struct xrt_device *xdev = xmbx->xdev;
	...
	mutex_lock(&xmbx->lock);
	if (xmbx->mailbox) {
		XMGNT_MAILBOX_PRT_REQ_SEND(xmbx, &req, leaf_req.xmir_sw_ch);
		/*
		 * mgnt should never send request to peer. it should send
...
static int xmgnt_mailbox_set_test_msg(struct xmgnt_mailbox *xmbx, char *buf, size_t len)
{
	mutex_lock(&xmbx->lock);
...
static ssize_t peer_msg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	struct xrt_device *xdev = to_xrt_dev(dev);
	struct xmgnt_mailbox *xmbx = xdev2mbx(xdev);
	int ret = xmgnt_mailbox_get_test_msg(xmbx, false, buf, &len);
			      ...
			      struct device_attribute *da, const char *buf, size_t count)
{
	struct xrt_device *xdev = to_xrt_dev(dev);
	struct xmgnt_mailbox *xmbx = xdev2mbx(xdev);
	int ret = xmgnt_mailbox_set_test_msg(xmbx, (char *)buf, count);
...
static struct attribute *xmgnt_mailbox_attrs[] = {
	&dev_attr_peer_msg.attr,
	NULL,
};

static const struct attribute_group xmgnt_mailbox_attrgroup = {
	.bin_attrs = xmgnt_mailbox_bin_attrs,
	.attrs = xmgnt_mailbox_attrs,
};

void *xmgnt_mailbox_probe(struct xrt_device *xdev)
{
	struct xmgnt_mailbox *xmbx = devm_kzalloc(DEV(xdev), sizeof(*xmbx), GFP_KERNEL);
	int ret;
	...
	ret = sysfs_create_group(&DEV(xdev)->kobj, &xmgnt_mailbox_attrgroup);
	if (ret) {
...
{
	struct xmgnt_mailbox *xmbx = (struct xmgnt_mailbox *)handle;
	struct xrt_device *xdev = xmbx->xdev;

	sysfs_remove_group(&DEV(xdev)->kobj, &xmgnt_mailbox_attrgroup);
	if (xmbx->mailbox)
		xleaf_put_leaf(xdev, xmbx->mailbox);
	if (xmbx->test_msg)
		vfree(xmbx->test_msg);
